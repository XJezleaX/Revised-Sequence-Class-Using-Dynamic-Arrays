// Provided by:   ____________Jezlea Ortega 0846__________
// Email Address: ____________jortgea010@citymail.cuny.edu________
...
//      Postcondition: sequence contructed will have the same items and capacity as source.
//
...
//
//   void advance( )
//     recondition: is_item returns true.
//     Postcondition: If the current item was already the last item on the
//     sequence, then there is no longer any current item. Otherwise, the new
//     current item is the item immediately after the original current item.P
//
...
namespace main_savitch_4
{
    ...
    public:
        ...
        value_type current( ) const;
    private:
        value_type *data; //Point to dynamic array
        size_type used;  //Current length in sequence
        size_type current_index; //index to current item
        size_type capacity; //Complete dynamic array capacity
    };
}
